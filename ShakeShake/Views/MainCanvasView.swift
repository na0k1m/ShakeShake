        ...
        GeometryReader { geometry in
            ZStack {
                ...
                .onChange(of: viewModel.currentDrawingPoint) { newPoint in
                    ...
                    // 스프레이 질감이 잘 보이도록 굵기를 적당히(40.0) 키움
                    let dynamicThickness: CGFloat = 40.0
                ...
                if let handPoint = viewModel.currentHandPoint, viewModel.sprayUIState != .hidden {
                    ...
                    // Assets에 'spray_active', 'spray_idle', 'spray_shake' 3가지 이미지를 추가해주세요!
                    Image(viewModel.sprayImageName)
                        .resizable()
                        .scaledToFit()
                        .frame(width: 150) // 에셋 크기는 필요에 따라 조절
                        // 에셋이 손에 자연스럽게 쥐어진 것처럼 보이게 오프셋 통일
                        .position(x: mappedX + 40, y: mappedY + 60)
                        // 흔들 때 약간의 율동감을 주는 이펙트를 넣으면 더 좋습니다.
                }
            } // ZStack End
        }
